...
with `0h (parallel)` and adds nothing to the total; a partial overlap logs the net (non-overlapping)
duration with a note saying which row billed the overlap. The Duration column always sums to
true wall-clock.
...
| 2026-06-11 | 11:14 | 11:25 | 0.1h | Session 2 block 3: T7 LLM nodes — llm.py factory, 4 prompt files, tool-calling widen (bind_tools, accept_results), review prechecks+verdicts, streamed narrate, run_demo CLI; 8 fake-model wiring tests. 81 tests green. **Live-LLM verify pending: needs ANTHROPIC_API_KEY in backend/.env.** Duration is net: raw 0.2h − 0.1h overlap with block 2 (overlap billed on block 2's row) |
...
| 2026-06-11 | 18:26 | 19:15 | 0.5h | Session 4: UX design brainstorm with browser mockups (Bo's 4 asks → 6 decisions): chat-hybrid (form in, transcript out), per-home sessions w/ background runs + localStorage, pinned hero valuation card w/ key-factors line + timestamps, address intake + deterministic community auto-fill + mismatch guards, `/api/ask` Q&A + what-if re-runs (visible diff → linked session). Design v3 approved; spec addendum written. Duration is net: raw 0.8h − 0.3h overlap with block 8 (overlap billed on block 8's row) |
...
**Total so far:** ~5.0h of 12h cap (5.0h build blocks + 2.0h reserve remaining).
