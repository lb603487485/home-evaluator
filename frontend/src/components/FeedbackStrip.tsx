...
export default function FeedbackStrip({ session, onSubmit }: Props) {
  ...
  if (fb) {
    return (
      <div className="rounded-xl border border-stone-200 bg-white p-2 text-xs text-stone-500">
        ...
        <span className="ml-1 text-stone-400">— logged for calibration</span>
      </div>
    )
  }
  ...
  return (
    <form
      className="flex flex-wrap items-center gap-2 rounded-xl border border-stone-200 bg-white p-2 text-xs"
      onSubmit={e => {
        e.preventDefault()
        if (!rating) return
        onSubmit({
    ...
    >
      <span className="font-semibold text-stone-600">Rate this valuation:</span>
      <span>
        {[1, 2, 3, 4, 5].map(n => (
          <button
      ...
      {rating > 0 && <span className="text-stone-400">{LABELS[rating]}</span>}
      <input
        className="w-44 rounded border border-stone-200 px-1.5 py-1"
      ...
      />
      <input
        className="min-w-36 flex-1 rounded border border-stone-200 px-1.5 py-1"
      ...
      <button
        ...
        className="rounded bg-orange-600 px-2.5 py-1 font-semibold text-white disabled:opacity-40"
      >
        Submit
      </button>
    </form>
