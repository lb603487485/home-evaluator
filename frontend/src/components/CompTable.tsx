            ...
            return (
              <Fragment key={c.address_key}>
                ...
                >
                  ...
                  <td className="px-3 py-2 text-stone-500">{c.sold_date}</td>
                  ...
                  <td className="px-3 py-2">
                    {review && (
                      <span
                        title={review.reason}
                        className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${review.unreviewed ? 'bg-stone-100 text-stone-500' : VERDICT_STYLES[review.verdict]}`}
                      >
                        ...
                        <div>
                          ...
                          {adj ? (
                            <table className="mt-1 text-xs">
                              <tbody>
                                <tr>
                                  <td className="pr-4 text-stone-500">sold price</td>
                                  <td className="text-right">{cad.format(adj.sold_price)}</td>
                                </tr>
                                {Object.entries(adj.adjustments).map(([term, amount]) => (
                                  <tr key={term}>
                                    <td className="pr-4 text-stone-500">{term}</td>
                                    <td className={`text-right ${amount > 0 ? 'text-emerald-700' : amount < 0 ? 'text-red-700' : 'text-stone-400'}`}>
                        ...
                        <div>
                          ...
                          <div className="mt-1 flex flex-wrap gap-1">
                            {Object.entries(c.sources).map(([field, source]) => (
                              <span key={field}
                                className={`rounded px-1.5 py-0.5 text-[10px] ${SOURCE_STYLES[source] ?? 'bg-stone-100 text-stone-600'}`}>
                                {field}: {source}
                              </span>
                            ))}
                          </div>
