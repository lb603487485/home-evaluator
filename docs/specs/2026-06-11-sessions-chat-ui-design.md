...
| S7 | **Conclusion UI in chat** (#12, style locked 2026-06-12 afternoon via browser mockups) | 0.5h | Narrative bubble splits on its three known markdown headings; if headings are missing (degraded narrate output) fall back to today's flat bubble — parsing never breaks rendering. **Conclusion** renders in a tinted callout (left orange accent bar + "Confidence X" pill), always visible; **How we got here** / **Caveats** become native `<details>` accordion rows, collapsed by default, caveat-count badge visible while collapsed. Hero card keeps the number; the bubble stops repeating it at full volume. Builds on S11's warm palette. |
...
| S11 | **Warm re-theme + 3-column layout** (approved 2026-06-12 afternoon via browser mockups — Listen360-inspired temperature, Bo's pick over the cool-slate recommendation; bills the reserve) | 0.75h | App-wide temperature swap using stock Tailwind tokens only — no tailwind.config surgery, no custom hex: `orange-50` cream page/transcript background, `stone-*` neutrals replace `slate-*`, `orange-600` as the single accent (primary buttons, links, pills, warm user-bubble tint), hero card stays dark with a warm orange figure. Mechanical class swaps across the ~7 components. Page restructure in the same pass: `[form] | [sessions] | [chat]` three full-height columns (Bo's structure; sessions leave the form's sidebar — ends the form/sessions height fight behind the earlier scroll fixes, and keeps live run badges always visible), width cap raised from `max-w-6xl` to full width. Consequence: README screenshots go stale → retake once after S7 lands (same pass as video prep). |

**Final build order (Bo, 2026-06-12 afternoon, amended on S11 approval): S5 → S9+S10 →
S11+S7+S8 → S6** (~4.0h boxed; S11's 0.75h bills the reserve). Two combined review units
(Bo's call): **S9+S10** — capture and report demo together; **S11+S7+S8** (1.5h unit,
built internally theme → conclusion UI → edit-in-form so everything is born in the warm
palette) — one visual pass, verified together. Functional work stays ahead of visual
polish, so if the 12h cap bites, polish is what drops — not the feedback loop. S7 before
S6 so popovers/handbook inherit the final chat styling. **Checkpoint protocol: build
strictly one unit at a time — after each unit, stop, hand to Bo for live verification,
and start the next only on his OK.**
